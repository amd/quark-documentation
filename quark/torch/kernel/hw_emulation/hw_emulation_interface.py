...
quant_scope_lib.define("quant_dequant_fp8_e4m3_with_scale(Tensor x, float scale) -> Tensor")


@impl(quant_scope_lib, "quant_dequant_fp8_e4m3_with_scale", "CompositeExplicitAutograd")
def quant_dequant_fp8_e4m3_with_scale(inputs: torch.Tensor, scale: float) -> Any:
    inputs_type = inputs.dtype
    inputs = inputs / scale
    inputs = torch.clamp(inputs, min=-448, max=448)
