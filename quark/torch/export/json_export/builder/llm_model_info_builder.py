...
from .llm_model_info import (CURRENT_VERSION, EmbeddingType, LayerNormType, QuantInfo, EmbeddingInfo, LayerNormInfo,
                             LinearInfo, ActInfo, AttentionInfo, MLPInfo, DecoderInfo, ModelInfo)


class LLMModelInfoBuilder:
    ...
    def get_scale(quantizer: FakeQuantize) -> Optional[torch.Tensor]:
        ...
        if "fp8" not in quantizer.dtype.name:
            return None

        max_val = quantizer.export_amax()
        if max_val is None:
            return None

        # Calculate the scale based on the max value of the observer
        scale = max_val.float() / quantizer.maxbound
        ...
        if scale.dim() == 0:
            ...
            raise NotImplementedError(f"scaling factor shape {scale.shape} not supported")

        # Ensure the scaling factor is positive
        return scale.detach().cpu()
    ...
    def build_linear_info(self, name: str, module: nn.Module) -> LinearInfo:
        ...
        if hasattr(module, "_input_quantizer") and module._input_quantizer is not None:
            ...
            quant_info.scale = LLMModelInfoBuilder.get_scale(quantizer)
            quant_info.zero_point = LLMModelInfoBuilder.get_zero_point(quantizer)
    ...
    def merge_scaling_factor(self, linear_group: List[LinearInfo]) -> None:
        ...
        for linear_info in linear_group:
            linear_info.weight_quant_info.name = weight_quant_name
    ...
    def virtual_merge_weight_matrix(self) -> None:
        ...
        for pattern in self.config.weight_merge_groups:
            pattern0 = pattern[0]
            for key in linear_keys:
                if fnmatch.fnmatch(key, pattern0):
                    ...
                    for other in pattern[1:]:
                        ...
                        if other_key in linear_keys:
                            key_list.append(other_key)
                    if key_list and len(key_list) > 1:
                        self.linear_weight_merged_groups.append(key_list)
...
class LlamaModelInfoBuilder(LLMModelInfoBuilder):
...
def create_llm_model_builder(model: nn.Module, model_type: str, model_dtype: torch.dtype,
                             config: JsonExporterConfig) -> LLMModelInfoBuilder:
