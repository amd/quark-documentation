...
class AmmoConverter:
    ...
    def _get_quant_type(self) -> str:
        ...
        if w_quantizer is None:
            ...
            if w_dtype is None:
                return QUANTIZATION_NONE
            elif "fp8" in w_dtype and w_qscheme == "per_tensor":
    ...
    def _to_quantized_weight(self, weight: torch.Tensor, weight_scale: torch.Tensor) -> torch.Tensor:
        weight = weight.cpu()
        weight_scale = weight_scale.cpu()

        if self.quant_type == QUANTIZATION_FP8:
            return quark.torch.kernel.quant_fp8_e4m3(weight, weight_scale).view(torch.int8)
        elif self.quant_type in [QUANTIZATION_INT4_AWQ, QUANTIZATION_W4A8_AWQ]:
            out_dim = weight.shape[0]
            assert (out_dim % 2 == 0), f"Out dimension {out_dim} should be an even number."
            in_dim = weight.shape[1]
            block_size = weight.shape[1] // weight_scale.shape[1]
            int8_tensor = ((weight / weight_scale[:, torch.arange(in_dim) // block_size]).round().clamp(-8, 7).to(
                torch.int8))
    ...
    def _build_qkv_weight(self, q_info: Dict[str, Any], k_info: Dict[str, Any], v_info: Dict[str, Any]) -> str:
        ...
        qkv_weight_name = AmmoConverter.create_qkv_name(q_weight_key)
    ...
    def _build_qkv_bias(self, q_info: Dict[str, Any], k_info: Dict[str, Any], v_info: Dict[str, Any]) -> Optional[str]:
        ...
        qkv_bias_name = AmmoConverter.create_qkv_name(q_bias_key)
    ...
    def _build_activation_scaling_factor(self, q_info: Dict[str, Any], k_info: Dict[str, Any],
        ...
        qkv_input_scale_name = AmmoConverter.create_qkv_name(q_input_scale_key)
    ...
    def _build_output_scaling_factor(self, q_info: Dict[str, Any], k_info: Dict[str, Any],
        ...
        qkv_output_scale_name = AmmoConverter.create_qkv_name(q_output_scale_key)
    ...
    def _build_weight_scaling_factor(self, q_info: Dict[str, Any], k_info: Dict[str, Any],
        ...
        qkv_weight_scale_name = AmmoConverter.create_qkv_name(q_weight_scale_key)
    ...
    def _get_kv_cache_scale(self, q_info: Dict[str, Any], k_info: Dict[str, Any],
        ...
        kv_cache_dtype = None
        if "fp8" in qkv_output_dtypes[0].lower():
            kv_cache_dtype = "FP8"
