...
-  **extra_options**: (Dictionary or None) Contains key-value pairs for
   ...
   -  **UseGPTQ**: (Boolean) If True, GPTQ algorithm will be applied to the 
      model. The default is False.
   -  **GPTQ_Params**: (Dictionary) A parameter used to specify the
      settings for GPTQ.
      ...
      -  **GroupSize**: (int) The group size used in GPTQ. The default is -1.
      -  **IsPertensor**: (Boolean) Determine whetehr to perform per-tensor quantization in GPTQ. The default is False.
      -  **MSE**: (Boolean) Determine whetehr to use MSE method to do data calibration in GPTQ. The default is False.
