...
Quantizing a floating-point model using Quark for PyTorch involves the following high-level steps:

1. **Load the Original Floating-Point Model**

   Begin by loading your model in its original floating-point format.

2. **Set the Quantization Configuration**

   Configure the quantization settings, including precision and calibration parameters.

3. **Define the Data Loader**

   Establish a data loader to supply input data for model calibration.

4. **Use Quark API for In-Place Replacement**

   Utilize the Quark API to replace the model's modules with quantized versions in-place.

5. **(Optional) Export the Quantized Model**

   Optionally, export the quantized model to other formats, such as ONNX.


Supported Features and Data Types
---------------------------------

Quark for PyTorch supports the following key features and data types:
...
:ref:`Advanced Quark Features and Usage in PyTorch <advanced-quark-features-and-usage-in-pytorch>`.

.. toctree::
   :maxdepth: 1
   :hidden:

   pytorch/pytorch_examples.rst